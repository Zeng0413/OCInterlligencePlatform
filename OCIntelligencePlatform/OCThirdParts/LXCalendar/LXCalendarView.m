...
-(void)setup{
    ...
    self.height = self.collectionView.oc_bottom;
...
-(void)layoutSubviews{
    ...
    self.calendarHeader.frame = CGRectMake(0, self.calendarWeekView.oc_bottom, self.width, 50*FITWIDTH);
}
#pragma mark---懒加载
-(LXCalendarHearder *)calendarHeader{
    if (!_calendarHeader) {
        _calendarHeader =[LXCalendarHearder showView];
        _calendarHeader.frame = CGRectMake(0, self.calendarWeekView.oc_bottom, self.width, 50*FITWIDTH);
...
-(LXCalendarWeekView *)calendarWeekView{
    if (!_calendarWeekView) {
        _calendarWeekView =[[LXCalendarWeekView alloc]initWithFrame:CGRectMake(0, 0, self.width, 62*FITWIDTH)];
...
-(UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        flow.itemSize = CGSizeMake(self.width/7, 40*FITWIDTH);
        _collectionView =[[UICollectionView alloc]initWithFrame:CGRectMake(0, self.calendarHeader.oc_bottom, self.width, 6 * 40*FITWIDTH) collectionViewLayout:flow];
