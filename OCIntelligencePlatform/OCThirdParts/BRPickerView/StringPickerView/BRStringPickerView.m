...
@property (nonatomic,assign) NSUInteger selectInteger;
// 主题色
...
+ (BRStringPickerView *)showStringPickerWithTitle:(NSString *)title
                                            frame:(CGRect)frame
                                       dataSource:(id)dataSource
                                  defaultSelValue:(id)defaultSelValue
                                     isAutoSelect:(BOOL)isAutoSelect
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    switch (self.type) {
        ...
        case BRStringPickerComponentMore:
        {
            ...
            if (self.isAutoSelect) {
                if(self.resultBlock) {
                    self.resultBlock([self.selectValueArr copy],self.selectInteger);
                }
            }
        }
            break;
...
- (void)clickRightBtn {
    ...
    if(_resultBlock) {
        if (self.type == BRStringPickerComponentSingle) {
            _resultBlock(self.selectValue,self.selectInteger);
        } else if (self.type == BRStringPickerComponentMore) {
            _resultBlock(self.selectValueArr,self.selectInteger);
        }
    }
}
