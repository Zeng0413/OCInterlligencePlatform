...
- (void)viewDidLoad {
    ...
    if (self.navigationController) {
        navigationAndStatusBarHeight = self.navigationController.navigationBar.frame.size.height + kStatusH;
    }
    self.tabBar.frame = CGRectMake(0,
                                   screenSize.height - kDockH,
                                   screenSize.width,
                                   kDockH);
	...
	Weak
	[[NSNotificationCenter defaultCenter] addObserverForName:@"changeSelectedVC" object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
        NSInteger index = [note.userInfo[@"selectedIndex"] integerValue];
		wself.tabBar.selectedItemIndex = index;
	}];
}
...
- (void)viewWillLayoutSubviews {
    ...
    self.tabBar.frame = CGRectMake(0,kViewH - kDockH, kViewW, kDockH);
}
