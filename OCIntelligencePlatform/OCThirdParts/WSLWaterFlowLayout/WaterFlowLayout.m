...
- (void)setFooterSpace:(NSInteger)footerSpace {
    if (_footerSpace != footerSpace) {
...
- (void)initAllCellHeight {
    ...
    //取得每一个cell的宽度
    CGFloat width = _cellWidth > 0 ? _cellWidth : itemAllWidth / _colum;
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)indexPath {
    if (elementKind == UICollectionElementKindSectionHeader) {
        ...
        attributes.frame = CGRectMake(0, _insetSpace.top, self.collectionView.width, _footerSpace);
        return attributes;
    }
}
...
- (CGSize)collectionViewContentSize {
    ...
    CGFloat width = _cellWidth > 0 ? (_cellWidth + _distance) * _colum : self.collectionView.width;
    //取出最长的列，将其高度定位长度
    CGFloat height = [self getLongColum];
    if (height < kViewH - kDockH) {
        height = kViewH - kDockH;
    }
...
- (void)initColumHeightArray {
    for (int i = 0; i < _colum; i ++) {
        ...
        //这两种形式一样的结果
        _columHeight[i] = @(_headerSpace + _insetSpace.top + _footerSpace);
    }
}
