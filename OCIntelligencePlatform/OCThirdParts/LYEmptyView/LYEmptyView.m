...
//按钮字体大小
#define kActionBtnFont  [UIFont boldSystemFontOfSize:16.f]
//按钮高度
...
- (void)setSubViewFrame{
    ...
    //获取self原始宽高
        ...
        self.ly_y = 70*FITWIDTH;
        self.centerX = emptyViewCenterX;
    //    self.center = CGPointMake(emptyViewCenterX, 70);
        ...
        if (self.contentViewY < 1000) {
            self.centerY += self.contentViewY;
        }
}
...
//    CGFloat cornerRadius = self.actionBtnCornerRadius ? self.actionBtnCornerRadius : 5.f;
    ...
    CGFloat btnWidth = textSize.width + horiMargin * 2 + 50;
    CGFloat btnHeight = height;
    btnWidth = btnWidth > contentMaxWidth ? contentMaxWidth : btnWidth;
    
    self.actionButton.frame = CGRectMake(0, contentHeight + subViweMargin + 30, btnWidth, btnHeight);
    [self.actionButton setTitle:btnTitle forState:UIControlStateNormal];
    self.actionButton.titleLabel.font = font;
    self.actionButton.backgroundColor = kAPPCOLOR;
    [self.actionButton setTitleColor:WHITE forState:UIControlStateNormal];
//    self.actionButton.layer.borderColor = borderColor.CGColor;
//    self.actionButton.layer.borderWidth = borderWidth;
    self.actionButton.layer.cornerRadius = btnHeight/2;
